...
//
// Returns v10 if a CONNECT frame and the accept-version header is missing.
//
...
func determineVersion(f *frame.Frame) (version Version, err error) {
	...
	if !isConnect && f.Command != frame.STOMP {
		err = fmt.Errorf("notConnectFrame")
		return
	}
	...
	if acceptVersion, ok := f.Header.Contains(frame.AcceptVersion); ok {
		...
		if isConnect {
			...
			err = fmt.Errorf("missing header [%s]", frame.AcceptVersion)
		}
	}
	return
}
